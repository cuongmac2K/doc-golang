...
func main() {
	a := time.Now()
	//b := getValueResouce("https://staging.bizflycloud.vn/api/iam/v2/projects?limit=100&sort=created_at:asc", "Get")
	...
	fmt.Println(b)
	//fmt.Println(getValueProject("b0120bb8-ae50-4cbd-aab7-65835ec0fbd2"))
	fmt.Println(time.Since(a))
}
func getValueResouce(resource string, method string) string {
	...
	for _, v := range res1 {
		...
		if strings.ToUpper(v.Type) == strings.ToUpper(method) && strings.Contains(resource, v.Url) {
