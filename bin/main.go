...
func sendMessage() {
	for i := 0; i < 10; i++ {
		...
		dataSend, _ := json.Marshal(ms)
		msg := &sarama.ProducerMessage{
			Topic: "devops-bizfly-activity-log-staging",
			Value: sarama.StringEncoder(dataSend),
		}
...
func InitKafkaProducer() (sarama.SyncProducer, error) {
	...
	brokerList := []string{"10.3.52.78:9093"} // Danh sách broker trong Kafka cluster
